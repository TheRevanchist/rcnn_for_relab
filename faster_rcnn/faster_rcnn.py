...
class FasterRCNN(nn.Module):
    ...
    def interpret_faster_rcnn(self, cls_prob, bbox_pred, rois, im_info, im_shape, nms=True, clip=True, min_score=0.05):
        # find class
        all_scores = cls_prob.data
        scores, inds = cls_prob.data.max(1)
        scores, inds, all_scores = scores.cpu().numpy(), inds.cpu().numpy(), all_scores.cpu().numpy()
        ...
        if nms and pred_boxes.shape[0] > 0:
            pred_boxes, scores, inds = nms_detections(pred_boxes, scores, 0.05, inds=inds)

        return pred_boxes, scores, self.classes[inds], inds, all_scores
        # return pred_boxes, scores, self.classes[inds]

    def detect(self, image, thr=0.05):
        ...
        pred_boxes, scores, classes, inds, all_scores = \
            self.interpret_faster_rcnn(cls_prob, bbox_pred, rois, im_info, image.shape, min_score=thr)
        return pred_boxes, scores, classes, inds, all_scores
